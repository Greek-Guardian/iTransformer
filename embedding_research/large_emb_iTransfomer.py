...
class iTransformer(nn.Module):
    ...
    """
    def __init__(self, configs, emb_model):
        ...
        if self.use_pretrained_emb:
            self.emb_enc = self.emb_model.ts2z
        else:
        ...
        if self.use_pretrained_emb:
            self.emb_dec = self.emb_model.z2ts
        else:
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.use_pretrained_emb:
            emb = self.emb_enc(x_enc.reshape(-1, L)).reshape(B, N, -1)
        else:
            if self.use_norm:
        ...
        if self.use_pretrained_emb:
            output = self.emb_dec(emb_out.reshape(B*N, -1)).reshape(B, -1, N)
        else:
            # B N E -> B N S -> B S N
