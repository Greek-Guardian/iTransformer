...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        discriminator_args = self.args
        discriminator_args.seq_len = self.args.pred_len
        discriminator = self.model_dict['Discriminator'].Discriminator(discriminator_args).float()

        if self.args.use_multi_gpu and self.args.use_gpu:
            model = nn.DataParallel(model, device_ids=self.args.device_ids)
            discriminator = nn.DataParallel(discriminator, device_ids=self.args.device_ids)
